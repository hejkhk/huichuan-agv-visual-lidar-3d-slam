...
CACHE_WS="${DUAL_3D_BUILD_ROOT:-$HOME/.cache/huichuan_agv_dual_3d_ws}"
...
if is_true "$ENABLE_STVL"; then
  ...
  NAV_COSTMAP_OVERRIDE="$SOURCE_WS/src/lidar_py/config/nav2_auto_mapping_jazzy.yaml"
fi
...
verify_navigation_source_contract() {
  ...
  controller_override="$SOURCE_WS/src/lidar_py/config/nav2_dual_3d_rpp_override.yaml"
...
[ -f /opt/ros/humble/setup.bash ] || [ -f /opt/ros/jazzy/setup.bash ] || die "ROS 2 was not found at /opt/ros/humble or /opt/ros/jazzy"
# shellcheck disable=SC1091
if [ -f /opt/ros/humble/setup.bash ]; then source /opt/ros/humble/setup.bash; else source /opt/ros/jazzy/setup.bash; fi
...
if is_true "$ENABLE_NAVIGATION"; then
  ...
  # In Jazzy, the nested costmap parameter services do not exist until the
