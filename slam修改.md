...
## 2026-08-05 V6.57：Gemini2 周期性点云停顿修复

- `dual_3d_2026-08-05_21-00-12` 中 RGB/Depth 原始流在两次停顿期间仍为约 15 Hz，点云处理仅需 12-24 ms；不是 USB 断流或 Jetson 算力不足。
- 原配置使用 `time_domain=device` 并每 10 秒执行 `timerSyncWithHost()`。Orbbec SDK 明确说明流运行时同步设备计时器可能让连续帧时间戳跳变；日志中的两次 8-12 秒停顿与该周期一致。
- Gemini2 改回官方默认 `global` 时间域，使用转换到主机时钟域的连续时间戳，不再在运行中周期性跳变设备时钟。
- Jetson 实测正常端到端帧龄为 145-185 ms，实时点云的头部时间戳年龄上限由临界的 150 ms 调整为 250 ms。真正断流仍由独立稳态接收看门狗停车，不因放宽该阈值而失去保护。
- 15 Hz RGB/Depth 近似同步窗口由 30 ms 调整为 45 ms，仍小于 66.7 ms 的单帧周期，同时避免相位抖动贴住阈值时 RTAB-Map 长时间收不到帧对。
...
## 2026-08-08 相似房间重定位活锁与 UI ROS 状态修复

- `dual_3d_2026-08-08_14-39-42` 证明地图服务器、雷达、里程计、Gemini2 和 Nav2 节点均已
  启动；失败点是精修后的前两名长期只有 `0.002-0.020` 分差，旧逻辑在进入多帧共识前就把
  每一帧丢弃，最终没有活动轨迹和 `map -> odom`。
- 保留原有严格唯一性门。严格通过的候选仍需 3 帧一致；得分至少 `0.76` 但单帧存在歧义的
  候选只能进入扩展时间共识，必须连续 6 个不同雷达时间戳稳定在 `0.35 m/12 deg` 内。任何
  位置或角度跳变都会清零该簇，不能用互相矛盾的候选凑满次数。
- 该日志中正确候选连续稳定在约 `(2.31,-2.42,180 deg)`，而竞争候选在多个房间和朝向之间
  跳变；新逻辑会等待稳定簇形成后再启动轨迹，随后仍执行 ACTIVE 轨迹、新鲜 TF、TF 前进、
  初值偏差和扫描得分五层验证，不是降低阈值后直接放行。
- UI 的本地 YAML/PGM 预览与 ROS 连接状态已拆开。本地地图可以先显示，但只有发现 ROS 图或
  收到真实话题才显示 ROS 已连接；`ui.log` 新增 `ROS_GRAPH_HEALTH map=N scan=N odom=N`，并在
  Python ROS 依赖缺失时直接记录具体导入错误。
