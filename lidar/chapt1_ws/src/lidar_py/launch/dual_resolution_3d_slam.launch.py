...
def generate_launch_description():
    ...
    navigation_launch = os.path.join(
        pkg_dir, "launch", "cartographer_auto_mapping_jazzy_launch.py")
    project_bt_dir = os.path.join(pkg_dir, "behavior_trees")
    rpp_override = os.path.join(
        pkg_dir, "config", "nav2_dual_3d_rpp_override.yaml")
    ...
    navigation = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            "bt_xml_file": os.path.join(
                project_bt_dir, "navigate_to_pose_jazzy.xml"),
            "through_bt_xml_file": os.path.join(
                project_bt_dir, "navigate_through_poses_jazzy.xml"),
        }.items(),
    )
