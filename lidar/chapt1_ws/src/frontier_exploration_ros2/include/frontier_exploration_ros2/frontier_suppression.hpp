...
{
  int dispatch_id{0};
  // True only after Nav2 has supplied a finite, positive path length. Nav2 may
  // briefly publish zero before the first global path reaches the BT blackboard;
  // that startup sentinel must not become the best-ever distance.
  bool has_feedback{false};
  double best_distance_remaining{0.0};
  double distance_at_last_progress{0.0};
  // Independent physical-motion fallback. A changing global path can make
  // distance_remaining jump or stay flat even while the robot is moving.
  bool has_pose_sample{false};
  double pose_at_last_progress_x{0.0};
  double pose_at_last_progress_y{0.0};
  int last_number_of_recoveries{0};
...
public:
  ...
  void note_goal_progress(
    int dispatch_id,
    double distance_remaining,
    int number_of_recoveries,
    int64_t now_ns);
  // Feed the current robot position into the watchdog. Net XY motion is an
  // independent indication of progress when Nav2 path-length feedback is stale.
  void note_goal_pose_progress(
    int dispatch_id,
    double x,
    double y,
    int64_t now_ns);
