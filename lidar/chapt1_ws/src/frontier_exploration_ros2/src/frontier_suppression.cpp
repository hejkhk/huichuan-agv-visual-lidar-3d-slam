...
void FrontierSuppression::note_goal_progress(
  int dispatch_id,
  ...
  int64_t now_ns)
{
  ...
  if (number_of_recoveries > progress.last_number_of_recoveries) {
    // A Nav2 recovery (for example Spin after a planning failure) is active work.
    // Give it a fresh watchdog window instead of canceling the parent action mid-recovery.
    progress.last_number_of_recoveries = number_of_recoveries;
    progress.last_meaningful_progress_ns = now_ns;
  }
  // NavigateToPose can briefly report zero before a global path has been
  // installed. If that startup sentinel becomes the historical minimum, every
  // later positive path length looks like regression and a moving robot is
  // canceled after the no-progress timeout.
  if (!std::isfinite(distance_remaining) || distance_remaining <= 0.0) {
    return;
  }
  ...
  if (!progress.has_feedback) {
    ...
    progress.last_number_of_recoveries = std::max(0, number_of_recoveries);
    progress.last_meaningful_progress_ns = now_ns;
    return;
  }
...
void FrontierSuppression::note_goal_pose_progress(
  int dispatch_id,
  double x,
  double y,
  int64_t now_ns)
{
  ...
  auto & progress = *state_.active_goal_progress;
  if (!progress.has_pose_sample) {
    // Seed the physical-motion baseline without granting an extra timeout
    // window. A motionless goal must still time out relative to dispatch.
    progress.has_pose_sample = true;
    progress.pose_at_last_progress_x = x;
    progress.pose_at_last_progress_y = y;
    return;
  }

  const double displacement = std::hypot(
    x - progress.pose_at_last_progress_x,
    y - progress.pose_at_last_progress_y);
  if (displacement >= config_.progress_epsilon_m) {
    progress.pose_at_last_progress_x = x;
    progress.pose_at_last_progress_y = y;
    progress.last_meaningful_progress_ns = now_ns;
  }
