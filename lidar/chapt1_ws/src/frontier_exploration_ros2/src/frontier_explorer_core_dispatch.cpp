...
void FrontierExplorerCore::try_send_next_goal()
{
  ...
  if (filtered_frontiers.empty() && !frontiers.empty()) {
    empty_frontier_confirmation_count = 0;
    empty_frontier_last_map_generation.reset();
    ...
    if (confirm_all_frontiers_suppressed_completion(*current_pose)) {
      return;
    }
  ...
  if (filtered_frontiers.empty()) {
    ...
    // Only count a genuinely new SLAM map. Scheduler ticks and costmap-only updates
    // must not turn one transiently empty extraction into a completed exploration.
    if (!empty_frontier_last_map_generation.has_value() ||
      *empty_frontier_last_map_generation != map_generation)
    {
      empty_frontier_last_map_generation = map_generation;
      empty_frontier_confirmation_count += 1;
      callbacks.log_info(
        "No frontiers detected; confirming completion on distinct map updates (" +
        std::to_string(empty_frontier_confirmation_count) + "/" +
        std::to_string(params.completion_confirmation_count) + ")");
    }

    if (empty_frontier_confirmation_count < params.completion_confirmation_count) {
      return;
    }

    if (!no_frontiers_reported) {
      callbacks.log_info(
        "No more frontiers found after " +
        std::to_string(empty_frontier_confirmation_count) +
        " consecutive distinct map updates");
...
void FrontierExplorerCore::consider_preempt_active_goal(const std::string & trigger_source)
{
  ...
  const auto active_goal_cost_status = params.goal_skip_on_blocked_goal && params.active_goal_blocked_cancel_enabled ?
...
void FrontierExplorerCore::feedback_callback(
  ...
  int dispatch_id)
{
  ...
  note_active_goal_progress(distance_remaining, number_of_recoveries);
