...
: params(std::move(params_in)), callbacks(std::move(callbacks_in))
{
  ...
  params.completion_confirmation_count = std::max(1, params.completion_confirmation_count);
  params.all_frontiers_suppressed_confirmation_count = std::max(
    1,
    params.all_frontiers_suppressed_confirmation_count);
  params.all_frontiers_suppressed_max_known_cell_growth = std::max(
    0,
