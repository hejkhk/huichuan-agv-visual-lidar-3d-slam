...
void FrontierExplorerCore::note_active_goal_progress(
...
bool FrontierExplorerCore::evaluate_active_goal_progress_timeout()
{
  ...
  // Use actual translation as a second progress signal. Nav2's path-length
  // feedback may start at zero or jump during replanning, but a robot that is
  // physically advancing must not be classified as stalled.
  const auto current_pose = callbacks.get_current_pose();
  if (current_pose.has_value()) {
    suppression->note_goal_pose_progress(
      current_dispatch_id,
...
void FrontierExplorerCore::reset_all_frontiers_suppressed_completion_tracking()
{
  all_frontiers_suppressed_confirmation_hits = 0;
  all_frontiers_suppressed_last_map_generation.reset();
  all_frontiers_suppressed_baseline_known_cell_count.reset();
}

bool FrontierExplorerCore::confirm_all_frontiers_suppressed_completion(
  const geometry_msgs::msg::Pose & current_pose)
{
  if (!params.complete_when_all_frontiers_suppressed || !map.has_value()) {
    reset_all_frontiers_suppressed_completion_tracking();
    return false;
  }

  if (all_frontiers_suppressed_last_map_generation.has_value() &&
    *all_frontiers_suppressed_last_map_generation == map_generation)
  {
    return false;
  }

  const auto & cells = map->map().data;
  const std::size_t known_cell_count = static_cast<std::size_t>(std::count_if(
      cells.begin(),
      cells.end(),
      [](int8_t value) {return value >= 0;}));

  all_frontiers_suppressed_last_map_generation = map_generation;
  if (!all_frontiers_suppressed_baseline_known_cell_count.has_value()) {
    all_frontiers_suppressed_baseline_known_cell_count = known_cell_count;
    all_frontiers_suppressed_confirmation_hits = 1;
  } else {
    const std::size_t allowed_growth = static_cast<std::size_t>(
      params.all_frontiers_suppressed_max_known_cell_growth);
    const std::size_t baseline = *all_frontiers_suppressed_baseline_known_cell_count;
    if (known_cell_count > baseline + allowed_growth) {
      // The map is still revealing meaningful new space. Start a fresh stable-map window.
      all_frontiers_suppressed_baseline_known_cell_count = known_cell_count;
      all_frontiers_suppressed_confirmation_hits = 1;
    } else {
      all_frontiers_suppressed_confirmation_hits += 1;
  ...
  callbacks.log_info(
    "All remaining frontiers are suppressed; confirming reachable-space completion on "
    "stable map updates (" + std::to_string(all_frontiers_suppressed_confirmation_hits) +
    "/" + std::to_string(params.all_frontiers_suppressed_confirmation_count) +
    ", known_cells=" + std::to_string(known_cell_count) + ")");

  if (all_frontiers_suppressed_confirmation_hits <
    params.all_frontiers_suppressed_confirmation_count)
  {
    return false;
  }

  callbacks.log_info(
    "All remaining frontiers stayed suppressed while the map was stable; "
    "reachable-space exploration is complete");
