...
TEST(FrontierSuppressionTests, NoProgressTimeoutTriggersCancelRequest)
{
  ...
  suppression.note_goal_progress(3, 10.0, 0, 0);
...
TEST(FrontierSuppressionTests, MeaningfulProgressResetsTimeout)
{
  ...
  suppression.note_goal_progress(7, 10.0, 0, 0);
  suppression.note_goal_progress(7, 9.4, 0, 3'000'000'000);
...
TEST(FrontierSuppressionTests, StartupZeroDistanceDoesNotPoisonProgressBaseline)
{
  FrontierSuppression suppression(FrontierSuppressionConfig{
    3, 2.0, 1.0, 90.0, 5.0, 0.5, 8, 4, 0.3});

  suppression.start_goal_progress_tracking(8, 0);
  suppression.note_goal_progress(8, 0.0, 0, 0);
  suppression.note_goal_progress(8, 10.0, 0, 1'000'000'000);
  suppression.note_goal_progress(8, 9.4, 0, 3'000'000'000);

  EXPECT_FALSE(suppression.mark_timeout_cancel_if_needed(8, 7'000'000'000));
  EXPECT_TRUE(suppression.mark_timeout_cancel_if_needed(8, 9'000'000'000));
}

TEST(FrontierSuppressionTests, PhysicalMotionResetsTimeoutWhenPathDistanceIsStale)
{
  FrontierSuppression suppression(FrontierSuppressionConfig{
    3, 2.0, 1.0, 90.0, 5.0, 0.5, 8, 4, 0.3});

  suppression.start_goal_progress_tracking(10, 0);
  suppression.note_goal_progress(10, 10.0, 0, 0);
  suppression.note_goal_pose_progress(10, 0.0, 0.0, 0);
  suppression.note_goal_progress(10, 10.0, 0, 3'000'000'000);
  suppression.note_goal_pose_progress(10, 0.6, 0.0, 3'000'000'000);

  EXPECT_FALSE(suppression.mark_timeout_cancel_if_needed(10, 7'000'000'000));
  EXPECT_TRUE(suppression.mark_timeout_cancel_if_needed(10, 9'000'000'000));
}

TEST(FrontierSuppressionTests, Nav2RecoveryResetsNoProgressTimeout)
{
  FrontierSuppression suppression(FrontierSuppressionConfig{
    3, 2.0, 1.0, 90.0, 5.0, 0.5, 8, 4, 0.3});

  suppression.start_goal_progress_tracking(9, 0);
  suppression.note_goal_progress(9, 10.0, 0, 0);
  suppression.note_goal_progress(9, 10.0, 1, 4'000'000'000);
  EXPECT_FALSE(suppression.mark_timeout_cancel_if_needed(9, 6'000'000'000));
  EXPECT_TRUE(suppression.mark_timeout_cancel_if_needed(9, 10'000'000'000));
}
...
TEST(FrontierSuppressionCoreTests, StableAllSuppressedFrontiersCompleteReachableSpace)
{
  int64_t now_ns = 1'000'000'000;
  int dispatch_calls = 0;
  auto core = make_suppression_core(&now_ns, &dispatch_calls, nullptr, nullptr);
  core->params.complete_when_all_frontiers_suppressed = true;
  core->params.all_frontiers_suppressed_confirmation_count = 3;
  core->params.all_frontiers_suppressed_max_known_cell_growth = 0;
  ...
  core->try_send_next_goal();
  EXPECT_EQ(core->all_frontiers_suppressed_confirmation_hits, 1);
  EXPECT_EQ(completion_calls, 0);

  // Re-running against the same raw map must not advance the terminal counter.
  core->try_send_next_goal();
  EXPECT_EQ(core->all_frontiers_suppressed_confirmation_hits, 1);

  core->map_generation += 1;
  core->try_send_next_goal();
  EXPECT_EQ(core->all_frontiers_suppressed_confirmation_hits, 2);
  EXPECT_EQ(completion_calls, 0);
...
TEST(FrontierSuppressionCoreTests, NoProgressTimeoutCanceledGoalCreatesSuppressedRegion)
{
  ...
  core->feedback_callback(10.0, 0, core->current_dispatch_id);
