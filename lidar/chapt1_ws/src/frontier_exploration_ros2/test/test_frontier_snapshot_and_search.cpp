...
TEST(FrontierSearchTests, ExplorationCompleteCallbackRunsWhenNoFrontiersRemain)
{
  ...
  core->params.completion_confirmation_count = 3;
  ...
  EXPECT_EQ(core->empty_frontier_confirmation_count, 1);

  // Re-running the scheduler against the same map must not advance confirmation.
  core->try_send_next_goal();
  EXPECT_EQ(completion_calls, 0);
  EXPECT_EQ(core->empty_frontier_confirmation_count, 1);
  ...
  core->try_send_next_goal();
  EXPECT_EQ(completion_calls, 0);
  EXPECT_EQ(core->empty_frontier_confirmation_count, 2);
  ...
  EXPECT_EQ(frontier_search_calls, 3);
