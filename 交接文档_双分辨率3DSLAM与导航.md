...
本分支面向 Ubuntu 24.04、ROS 2 Jazzy、RTX 3060 和 Gemini2 深度相机，实现：
...
```text
lidar/chapt1_ws/src/lidar_py/config/nav2_auto_mapping_jazzy.yaml
```
...
| `lidar_py/config/nav2_auto_mapping_jazzy.yaml` | Nav2 主参数和真实车体 footprint |
...
| `lidar_py/behavior_trees/navigate_to_pose_jazzy.xml` | 路径失效/周期重算和受控倒车、双向旋转脱困 |
...
- 本次在 Windows 完成静态检查，未进行 ROS 2 Jazzy 编译和实车验证。
